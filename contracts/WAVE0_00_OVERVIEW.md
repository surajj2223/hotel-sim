...
|----------|-------------|--------|-----------|---------------|------------------|
| `WAVE0_01_SCHEMA.sql` | SCH-/ENM-/INV- (all) | **FROZEN** | pre-existing | [RX-002](refactor-x/RX-002-line-amount-strategy-owned.md) (`SCH-022 chk_line_amount` forward spec only — verification record stands) | `V1__wave0_schema.sql` applied; SCH-001..071 referenced in entities; entity tests pass. Change only via a new Flyway migration. |
...
| `WAVE0_02_OPENAPI.yaml` | API-008 `lineCoverage` (Slice S2) | **DRAFT** | (this slice) | — | additive optional field; backward-compatible; exposes WHK-016 over HTTP. Optional `lineCoverage` array on `PaymentLinkCreateRequest` + derived read-only `revenuePosted` on `BookingLineResponse`. Omitted = folio-wide (WHK-012). API-008 FROZEN status unchanged; amendment flagged in `WAVE0_02` (`x-requirements-stage2` API-008 `amendment` block), awaiting Desk sign-off to freeze. |
