...
| 0.8 | 2026-06-14 | §1b: added **WHK-016 (DRAFT)** row — scoped payment→line allocation (Stage 4 Slice 1), additive over WHK-012; new `payment_line` table via additive `V3` migration. Drafted in `WAVE0_03 §5.1`; `WAVE0_02` API-008 DTO unchanged (scoping wired at the service layer; HTTP exposure deferred to a separate API-contract change). Not frozen — awaiting Desk sign-off. |
