...
| 2.4 | 2026-06-30 | §1b: **API-016 `getRevenue`** + **API-017 `listUnpaidBookings`** flipped **DRAFT→FROZEN** on Desk sign-off. No code — contract authorisation only; implementation slice may now proceed. Flipped the eight DRAFT markers in `WAVE0_02_OPENAPI.yaml` (line-1 header, §27 reports-tag prose, two path comments, two schema comments, `x-requirements-reports` block-level `status`) and removed the trailing "NOT self-frozen — awaiting Desk sign-off" clause from the two §1b rows (status cells DRAFT→FROZEN). No schema/path/DTO/field change; status text only. |
